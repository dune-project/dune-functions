...
For two sets $R$ and $\Omega$ we denote by $R^\Omega = \{f:R \to \Omega\}$
...
scalar-valued polynomials of degree at most $k$ on $\Omega$.
...
\todosander{Dieser letzte Satz stimmt leider in seiner Einfachheit nicht mehr---mittlerweile
haben wir ja drei Arten von Bäumen.}
\todograeser{Besser so?}
...
of such spaces.
Let $B = \{\lambda_i\}_{i=0}^{n_1-1}$ and $C = \{\gamma_i\}_{i=0}^{n_2-1}$ be two scalar function space bases
and let $\mathbf{e}_1$, $\mathbf{e}_2$ be the canonical basis vectors in $\R^2$.
Then we define the product of the bases $B$ and $C$ as
\begin{align*}
(B,C)
 & \colonequals
    \mathbf{e}_1 \otimes B \cup \mathbf{e}_2 \otimes C \\
    &= \big\{ \mathbf{e}_1 \otimes \lambda \st \lambda \in B \big\}
    \cup \big\{ \mathbf{e}_2 \otimes \gamma \st \gamma \in C \big\} \\
 & =
 \big\{(\lambda_0,0), (\lambda_1,0), \dots, (\lambda_{n_1-1},0) \big\}
   \cup \big\{(0,\gamma_0), (0,\gamma_1), \dots, (0,\gamma_{n_2-1})\big\}.
\end{align*}
These basis functions take values in $\R \times \R = \R^2$.
More generally, if $B$ and $C$ are bases with ranges $\R^{r_1}$
and $\R^{r_2}$, respectively, then
\begin{equation}
\label{eq:basis_product}
    (B,C)
 \colonequals
 \big\{(\lambda_0,\mathbf{0}), (\lambda_1,\mathbf{0}), \dots, (\lambda_{n_1-1},\mathbf{0})\big\}
 \cup
 \big\{(\mathbf{0}, \gamma_0), (\mathbf{0},\gamma_1), \dots, (\mathbf{0}, \gamma_{n_2-1})\big\}
\end{equation}
is the natural basis of the space $\operatorname{span} B \times \operatorname{span} C$.
Its basis functions take values in $\R^{r_1} \times \R^{r_2} = \R^{r_1+r_2}$.
In particular, the symbol $\mathbf{0}$ in the formula above denotes the constant $\R^{r_1}$ and
$\R^{r_2}$-valued zero functions.

Similarly to the case of function spaces, bases can be interpreted as trees.
This time, the individual basis functions form the tree leafs, and scalar or otherwise irreducible bases are
notated as trees with a single root and no further non-leaf roots.
\begin{center}
    \begin{tikzpicture}[
            level/.style={
                sibling distance = (3-#1)*0.5cm + 1cm,
                level distance = 1.5cm
            }
        ]
        \node [treenode] {$B$}
            child{ node [treenode] {$\gamma_0$} }
            child{ node [treenode] {$\gamma_1$} }
            child{ node [treenode] {$\gamma_2$} }
            child{ node [] {$\dots$} }
            child{ node [treenode] {$\gamma_{n-2}$} }
            child{ node [treenode] {$\gamma_{n-1}$} };
    \end{tikzpicture}
\end{center}
The operation of taking two bases $B$ and $C$, and joining them to form a new basis $(B,C)$
as in~\eqref{eq:basis_product} is interpreted as taking the two trees corresponding to $B$ and $C$,
and making them the (unique) children of a new root that represents $(B,C)$.
As an example, Figure~\ref{fig:taylor_hood_basis_tree} shows the tree
of the Taylor--Hood basis.  Observe how the tree spanned by the inner nodes of that tree
is isomorphic to the tree of the Taylor--Hood \emph{space}, shown in Figure~\ref{fig:taylor_hood_space_tree}.
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node [treenode] {$(P_2 \times P_2 \times P_2) \times P_1$}
                child{ node [treenode] {$P_2 \times P_2 \times P_2$}
                    child{ node [treenode] {$P_2$}
                        child{ node [basisnode] {$\lambda_0^2$} }
                        child{ node [] {$\dots$} }
                        child{ node [basisnode] {$\lambda_{n_2-1}^2$} }
                    }
                    child{ node [treenode] {$P_2$}
                        child{ node [basisnode] {$\lambda_0^2$} }
                        child{ node [] {$\dots$} }
                ...
                child{ node [treenode] {$P_1$}
                    child [sibling distance=1cm] { node [basisnode] {$\lambda_0^1$} }
                    child [sibling distance=1cm] { node [] {$\dots$} }
                    child [sibling distance=1cm] { node [basisnode] {$\lambda_{n_1-1}^1$} }
                };
        \end{tikzpicture}
    \end{center}
...
basis vectors of the Taylor--Hood basis of Figure~\ref{fig:taylor_hood_basis_tree}.
...
to the leaf nodes, and the multi-index digits label the \emph{edges} of the tree.  Prefixes can be
interpreted as path from the root to a given node.
\todosander{Hmm.  Ist das redundant?  Wir nehmen ja an dass wir \emph{geordnete} Bäume haben,
d.h., die Kinder eines Knotens haben ohnehin immer eine feste Ordnung $0,\dots, d-1$.
Die Ziffern des Multi-Index geben aber immer gerade diese Ordnung der Kinder wieder.}
        ...
        \begin{tikzpicture}[
            ...
            \node [treenode] {$[]$}
                child{ node [treenode] {$[0]$}
                    child{ node [treenode] {$[0,0]$}
                        child{ node [basisnode] {$[0,0,0]$} edge from parent node[left] {$0$}}
                        child{ node [] {$\dots$} }
                        child{ node [basisnode] {$[0,0,n_2]$} edge from parent node[right] {$n_2$} }
                        edge from parent node[above left] {$0$}
                    }
                    child{ node [treenode] {$[0,1]$}
                        child{ node [basisnode] {$[0,1,0]$} edge from parent node[left] {$0$}}
                        child{ node [] {$\dots$} }
                        child{ node [basisnode] {$[0,1,n_2]$} edge from parent node[right] {$n_2$} }
                        edge from parent node[left] {$1$}
                    }
                    child { node [treenode] {$[0,2]$}
                        child{ node [basisnode] {$[0,2,0]$} edge from parent node[left] {$0$}}
                        child{ node [] {$\dots$} }
                        child{ node [basisnode] {$[0,2,n_2]$} edge from parent node[right] {$n_2$} }
                        edge from parent node[above right] {$2$}
                    }
                    edge from parent node[above left] {$0$}
                }
                child{ node [treenode] {$[1]$}
                    child [sibling distance=1cm] { node [basisnode] {$[1,0]$} edge from parent node[above left] {$0$} }
                    child [sibling distance=1cm] { node [] {$\dots$} }
                    child [sibling distance=1cm] { node [basisnode] {$[1,n_1]$} edge from parent node[above right] {$n_1$} }
...
index tree.
\todosander{Notation: Der Buchstabe $I$ wird für zu viele Dinge verwendet.}

\begin{definition}
  Let $M$ a finite set and $I:M \to \mathcal{N}$ an injective map whose range
  $I(M)$ forms an index tree.
...
\todosander{Wofür brauchen wir Uniformität?}
\end{definition}
...
\end{align*}
with scalar-valued basis functions
\begin{equation*}
...
\begin{figure}
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node [treenode] {$[]$}
                child{ node [treenode] {$[0]$}
                        child [sibling distance=2.5cm] { node [treenode] {$[0,0]$}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,0,0]$} edge from parent node[above left] {$0$}}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,0,1]$} edge from parent node[left] {$1$}}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,0,2]$} edge from parent node[above right] {$2$}}
                            edge from parent node[above left] {$0$}
                        }
                        child [sibling distance=2.5cm]{ node [] {$\dots$} }
                        child [sibling distance=2cm]{ node [treenode] {$[0,n_2]$}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,n_2,0]$} edge from parent node[above left] {$0$}}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,n_2,1]$} edge from parent node[left] {$1$}}
                            child [sibling distance=1.5cm] { node [basisnode] {$[0,n_2,2]$} edge from parent node[above right] {$2$}}
...
that do not mimic the structure of the basis tree:
...
\begin{figure}
 ...
 \caption{Two matrix occupation patterns for different indexings of the Taylor--Hood bases.
   ...
   \todosander{Das linke Bild gehört zu einem 2d-Gebiet, wir behandeln im Text aber immer
...
natural number can be represented by an index tree with $3(n_2+1) + (n_1+1)$ leaf nodes all
...
Consider a tree of basis functions. We want to construct an indexing for this tree, that is
an index tree $\mathcal{I}$ and a map $I$ from the set of basis vectors to the leafs of the
index tree. This construction proceeds recursively.
Remember first that basis trees are ordered, i.e., the children of each tree node
have a fixed given ordering.  This means in particular that all leaf nodes have a fixed
given ordering with respect to their direct fathers.
We then decorate each inner node $A$ of the basis tree with a rule
that specifies how the index trees of the subtrees of $A$ are combined to form an index tree
for the basis subtree rooted at $A$. Of the large set of possibilities, we single out four rules
that appear frequently in applications.

\begin{itemize}
 \item \textbf{BlockedLexicographic}: Introduce a new node $A_I$ in the index subtree, and make
   the roots of the index subtrees children of $A_I$. Order them following their corresponding
   ordering given in the basis tree.
...
\end{tabular}
   ...
   Exclusive use of this rule will lead to an index tree that is isomorphic to the basis tree.

 \item \textbf{FlatLexicographic}: Instead of introducing a new root node, merge the existing
   index subtree roots into a single node.
   \todosander{Ergeben sich die Indizes damit automatisch?  Oder müssen wir hier explizit
     angeben wie sich die Offsets berechnen?}
...
\todosander{Ein Beispiel für den Fall dass die unterschiedlichen Teilbäume nicht die gleiche Art von Indizes verwenden?}


\end{itemize}

The previous rules have been called \emph{lexicographic}, by which we mean that all degrees of
freedom of the first subtree of $A$ appear before all degrees of freedom of the second subtree,
and so on.
\todosander{Streng genommen ist dies das erste Mal dass wir von Reihenfolgen sprechen.
  Das sollten wir noch etwas formalisieren.}
These rules can be applied in all situations. For more rules we have to restrict the set
of inner nodes.
\begin{definition}
An inner node will be called \emph{power node}, if all of its children are identical subtrees.
An inner node that is not a power node is called \emph{composite node}.
\end{definition}
In particular, each subtree of a \emph{power} node has the same number of basis functions.
It is therefore possible to interleave the degrees of freedom.

\todosander{Man könnte darüber nachdenken das zu verallgemeinern:  Wenn z.B. ein Teilbaum
genau doppelt so viele Basen hat wie der andere, dann könnte man immer abwechselnd zwei Basen aus dem
einen Teilraum nehmen und dann eine aus dem anderen.  Ich wäre eher dagegen über so etwas
nachzudenken, wenn ich es nicht in einer Anwendung brauchen würde...}

\begin{itemize}
 \item \textbf{FlatInterleaved}:
...
\end{tabular}
 ...
 \item \textbf{LeafBlockedInterleaved}:
...
\todosander{Hier kommt noch alter Text, der weggeschmissen oder irgendwo eingebaut werden muss.}


It is then most natural
...
To construct an admissible ordering
...
\end{equation*}
The local basis forms a tree that is isomorphic to a subtree of the full global basis tree.

From this natural isomorphism $\imath_e$ into the full tree of global basis functions follows
naturally a global indexing of the local basis functions
\begin{equation*}
 \Lambda_\text{local} \ni \lambda \mapsto \mathcal{I}
 \qquad
 I_\text{local}(\lambda) \colonequals I(\imath_e(\lambda)).
\end{equation*}
This is the map that associates shape functions on a given grid element $e$ to global degrees of freedom.

In addition, the basis functions in the local tree have two further indices.
...
This allows to use nested types for element stiffness matrices and load vectors.
As explained in Chapter~\ref{sec:function_space_bases_implementation}, however,
the \dunemodule{dune-functions} \emph{implementation} is fairly restrictive here,
and only allows flat lexicographic indices here.
...
you can write
\todosander{Ich schlage vor dass wir eine Regel für die Namensgebung angeben:  Zum Beispiel immer
...
\begin{center}
 \begin{tabular}{r|l}
  ...
  \cpp{RaviartThomasBasis} & \cpp{raviartThomas<std::size_t k, BasicType basic_type, class size_type=std::size_t>} \\
  \cpp{BrezziDouglasMariniBasis} & \cpp{brezziDouglasMarini<std::size_t k, BasicType basic_type>} \\
...
to be either factory tags for basis objects, like \cpp{lagrange<1>()} or \cpp{raviartThomas<k, GeometryType::BasicType::cube>()},
