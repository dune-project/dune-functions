...
by exporting a {\tt Tree} object. This implements a tree
...
Basis for all these nested product spaces can be represented
...
value in this rage,i.e., the result of {\tt localIndex(0)}
is also accessible via the {\tt offset()} method.
The result of these methods is undefined if the
local view containing the tree is in unbound state.
All these indices are of type {\tt size\_type}.
...
exports its path in the full tree via the {\tt treePath()}
...
type {\tt TreePath} will differ from node to node
in general. If data needs to be attached to nodes,
the {\tt treeIndex()} can be used. The latter returns
...
to via the {\tt element()} method. Probably most important
is the {\tt finiteElement()} method that gives access
...
The {\tt LocalIndexSet} as returned by {\tt GlobalBasis::localIndexSet()}
provides access to global multi-indices for the
local basis functions reachable from a local view.
To this end the {\tt LocalIndexSet} must
first be bound to the {\tt LocalView} using
the {\tt bind(LocalView)} method. Similar to the
local view there is an {\tt unbind()} method
and the bound on local view can be accessed
using the {\tt localView()} method.
...
of a given {\tt globalBasis} at a given {\tt element}.
If we want to compute the global index of the {\tt i}-th
...
such that the {\tt tree()} method directly returns a {\tt LeafBasisNode}.
In this case {\tt localIndex} coincides with {\tt i}.
In the more general case of a nontrivial product space
we must first retrieve the leaf node corresponding to
the desired factor in the product space. Assuming that
the path to this node is given as {\tt treePath},
the line to obtain the leaf node
...
\subsection{Implementors interface of a {\tt NodeFactory}}
\subsection{Implementors interface of a {\tt NodeIndexSet}}
