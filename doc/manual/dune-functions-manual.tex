...
\todograeser{I'm aware of several real word applications. However, there is not really a generic scheme
  as the onces discussed above. Implementing this amounts in allowing user-defined index-merging strategies.
  I'm all in favor of doing this. But unless it's implemented, we should drop the todo remark here.}
...
\todograeser{IMHO we should merge \cpp{LocalIndexSet} into \cpp{LocalView}!
I prepared a merge request implementing this and discussing the reasons:
\url{https://gitlab.dune-project.org/staging/dune-functions/merge_requests/118}}
...
\todosander{Rename \cpp{namespace BasisBuilder} to \cpp{namespace BasisFactory}!}
...
For each strategy \cpp{FooBar} there is a function \cpp{BasisBuilder::fooBar()} creating
the flag in the header \file{functionspacebases/basistags.hh}.

\todograeser{Remove old text below}
Four merging strategies are possible.  They differ by the ordering
...
\subsection{Implementor interface for global bases}
