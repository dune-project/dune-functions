...
\todosander{Sollten auch Leaf Nodes eine \cpp{child}-Methode bekommen?
Einfach für die Konsistenz? (Brauchen tue ich sie nicht.)}
\begin{lstlisting}[style=Interface]
...
descendants. The path can either be passed
as a \cpp{Dune::TypeTree::HybridTreePath} object (see Section~\ref{sec:multi_indices} below),
or as a sequence of indices. Indices referring
to children of a power node can be passed as integer values.
...
Continuing the example of the previous section, suppose that \cpp{localView} is a
local view of the \cpp{TaylorHoodBasis} localized to a particular grid element.
The leaf node for the second velocity
component can be obtained by the following.
\begin{lstlisting}[style=Example]
// Import namespace with index constants _0, _1, _2, etc
using namespace Indices;

// Obtain the leaf node for second component of the velocity tree
const auto& node = localView.tree().child(_0, 1);
\end{lstlisting}

The index constant \cpp{\_0} from the namespace \cpp{Dune::Indices} is used
...
Within the velocity subtree, all children are identical, and can be
accessed by a run-time integer.

The same result can be achieved by creating a \cpp{HybridTreePath} object first:
%
\begin{lstlisting}[style=Example]
...
\end{lstlisting}
\todosander{Die dritte (globale) Methode taucht bisher nur in diesem Beispiel auf.
  Wir sollten sie entweder ordentlich beschreiben, oder komplett löschen.}

At each leaf node, the set of all corresponding shape functions is available.
...
localBasis.evaluate({0,0}, values);
for (auto v : values)
  std::cout << v << std::endl;
\end{lstlisting}

To relate the leaf-local index of a shape function to its local index in the set
of all shape functions on a given element, the local index needs to be computed
from the leaf-local index.
...
and the result is the index within the full tree.
In other word, the method \cpp{localIndex(size\_type)}
...
general multi-indices, the \dunemodule{dune-functions} module only supports one
specific type of flat index here to keep the implementation simple.

While \cpp{LocalFiniteElement} objects are only available at leaf nodes,
the following methods work at every node in the tree.
The method
\begin{lstlisting}[style=Interface]
using size_type = @@<implementation defined>@@;
size_type size() const;
...
To obtain its position in the tree, each node
...
On the one hand, they are used as global indices to identify individual
basis functions of a function space bases.
...
of the tree.  Local indices of degrees of freedom could in principle also
be multi-indices, but \dunemodule{dune-functions} forces those to be flat.
Hence standard integer types are used for local indices.

From an implementation point of view, the two types of multi-indices behave
differently.  Global indices are run-time constructs.  A single C++ type
represents all elements of the set of global basis functions, even if they
are not all of the same length.  On the other hand,
the multi-indices needed to access the nodes in a tree may have to be
compile-ttype values, because the tree structure is statically encoded
and requires compile-time information to be accessed.
...
\end{lstlisting}
\todosander{Hier müsste doch auch noch \cpp{auto operator[](integral_constant)} stehen.}
...
not exceeding \cpp{k} entries.  A run-time selectable size is needed
if the multi-indices of the
function space basis are non-uniform, i.e., if multi-indices
of different lengths appear in the same index set.
...
\todosander{Was ist jetzt mit Spezialtypen für uniforme Indizes der Länge $>1$?
 Implementieren wir das noch?}
...
All multi-index implementations described above allow for dynamic entries
only. Since any node in the local ansatz tree can have children of different
type, it is not possible to access those children using a dynamic runtime
index. Instead the child index in a composite node has to encoded in a static
way as \cpp{Dune::index_constant<i>}.
On the other hand, in the special case of a power node, all children have
...
the multi-indices used identity tree nodes are implemented using the
...
This class has a compile type size and the types of the index entries
...
the value is static, if the type is \cpp{Dune::index_constant<i>}.
For example, the multi-indices to access the leaf nodes corresponding
...
\begin{itemize}
  \item
    ...
    should be interpreted as a function form this function space.
  ...
  \item
    For some given function space basis and function compute
    the interpolation of the function into the function
    space and store the result into a coefficient vector.
\end{itemize}
...
When writing a finite element functions given by a coefficient vector
with respect to a function space basis into some output format,
this typically requires to evaluate the finite element function.
...
\todograeser{Example, combination with \cpp{subspaceBasis}}
