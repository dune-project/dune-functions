...
and let $\mathbf{e}_0$, $\mathbf{e}_1$ be the canonical basis vectors in $\R^2$.
Then we define the product of the bases $B$ and $C$ as
\begin{align*}
(B,C)
 & \colonequals
    \mathbf{e}_0 \otimes B \cup \mathbf{e}_1 \otimes C \\
    &= \big\{ \mathbf{e}_0 \otimes \lambda \st \lambda \in B \big\}
    \cup \big\{ \mathbf{e}_1 \otimes \gamma \st \gamma \in C \big\} \\
 & =
...
Consider a tree of finite element space bases. We want to construct an indexing for this tree, that is
...
To describe the recursive construction remember first all trees are ordered,
i.e., the children of each tree node have a fixed given ordering and thus
a well-defined zero-based index with respect to their direct fathers.
In the following we assume that $\Lambda$ is the set of all basis functions
corresponding to a node in the tree of finite element spaces, i.e.,
we can associate $\Lambda$ to the space $\operatorname{span} \Lambda$
within this tree.

If $\Lambda$ corresponds to a leaf node
in the tree of finite element spaces, i.e., $\operatorname{span} \Lambda$ is
an elementary space of the product space construction,
...
If $\Lambda$ corresponds to the root or any other interior node,
then it takes the form
\begin{align*}
  \Lambda = (\Lambda_0, \dots, \Lambda_{m-1})
          = \bigcup_{i=0}^{m-1} \mathbf{e}_i \otimes \Lambda_i
\end{align*}
where $\Lambda_i$ is the set of basis functions of the $i$-th child
space $\operatorname{span} \Lambda_i$ of the product space
\begin{align*}
  \operatorname{span} \Lambda = \times_{i=0}^{m-1} \operatorname{span} \Lambda_i
...
In the following we describe for strategies to achieve this, that
...
\begin{itemize}
  ...
  \item \textbf{FlatLexicographic}: This strategy introduces
    ...
    \begin{align*}
      L_i = \sum_{j=0}^{i-1} |\iota_j(\Lambda_j)|_{()}.
    \end{align*}
...
We now consider the Taylor--Hood basis
\begin{align*}
 \Lambda_\text{TH}
  = ((\Lambda_{P_2}, \Lambda_{P_2}, \Lambda_{P_2}), \Lambda_{P_1})
\end{align*}
as an example. For the bases $\Lambda_{P_1}, \Lambda_{P_2}$
...
$\Lambda_{\text{TH}}$.
In the following table we list all combinations, where the label $X(Y)$
