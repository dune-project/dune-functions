...
However, the information is generally not
...
i.e., the number of direct children of the node $P$ in the index tree.
...
Each \cpp{GlobalBasis} implementation class is expected to export the types it uses
for multi-indices.  There is typically more than one type in use, and in particular
\cpp{MultiIndex} (the type used for global multi-indices of degrees of freedom)
will differ from \cpp{SizePrefix}.  Multi-index implementations are discussed
in Section~\ref{sec:multi_indices}.
...
To obtain a localization of this basis to an element (i.e., a \dune grid entity of codimension~0),
write
\begin{lstlisting}{style=example}
// create a LocalView
...
In addition, a bound \cpp{LocalView} object provided information
...
to children of a power node can be passed as
an integer value, e.g., as \cpp{std::size_t}.
Indices referring to children of a composite node
have to be passed statically as \cpp{Dune::index_constant<i>}
(which is a shortcut for \cpp{std::integral_constant<std::size_t, i>}).
...
tree.
\todosander{Umschreiben: Das folgende gilt nur für Leaf Nodes.}
...
to via the \cpp{element()} method.
\todosander{Auch hier fände ich es konsistenter wenn es die Methode \cpp{element}
für alle Knoten gäbe, nicht nur für Leafs.}
...
the desired factor in the product space.
...
It has to be noted that while the trees of global and local basis
functions is encoded statically in C++ type information,
the index tree, i.e., the set of multi-indices use to label global
degrees of freedom is a pure run-time construct.
Thus the entries of the global multi-indices are always dynamic.
