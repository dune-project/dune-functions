...
As an example, suppose that \cpp{basis} is an object of type \cppbreak{Dune::Functions::TaylorHoodBasis},
...
or STL containers directly, but all others have to be wrapped in an approprate backend
explicitly.
...
\end{lstlisting}
\todosander{Nachdem wir auf der vorigen Seite erklärt haben dass es für Ranges
einen Default-Mechanismus gibt (den der normale Leser beim ersten Lesen nicht
verstehen wird), finde ich es hier (in meiner Rolle als Erstleser) etwas
überraschend dass man die Range hier dennoch explizit angeben muss.
Können wir einen erläuternden Satz dazuschreiben?}
...
for B-splinef  bases, and therefore the standard interpolation functionality cannot be used with the
...
where the function is to be projected on the span of the basis, yielding
