...
Consider linear range spaces $R_0,\dots,R_{m-1}$ of function spaces $R_0^\Omega,\dots,R_{m-1}^\Omega$
...
\end{align}
Note $\mathbf{e}_i \otimes \Lambda_i$ is understood element-wise
and that the 'tagged union' symbol $\sqcup$ used here
...
to the leafs we will---in slight abuse of notation---denote the set of multi-indices
...
together with the three shape-function index maps
$\iota^{\text{leaf-local}}_{\hat{\Lambda}|_e}$,
$\iota^{\text{leaf}\to\text{local}}_e$,
...
As an example, if the power function space generated by the basis
\begin{lstlisting}[style=Example]
...
For more complex nested bases this approach is naturally extended:
...
In the special case that the range type does not provide an \cpp{operator[]}
it is directly used for all leaf nodes in the ansatz space which allows
to interpolate a scalar function into all components of a basis.
Notice that the described behavior is the default implementation
and can be customized. However, we will only consider the default implementation
in the following.
