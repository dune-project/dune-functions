...
\todograeser{%
    Do we require that \texttt{nodeFactory()} and \texttt{NodeFactory} exists?
    Do we require that each global basis is implemented using a factory?
    Do we require that GlobalBasis is always a DefaultGlobalBasis parametrized with the node factory?
}
...
\begin{itemize}
 ...
 \item \cpp{TaylorHoodBasis}: The Taylor--Hood bases described in Chapter~\ref{sec:finite_element_trees}.
   ...
   \todograeser{Es war Christian wichtig, dass man nicht-triviale Basen auch manuell,
    d.h. ohne \texttt{PowerNodeFactory}, implementieren kann. Für diesen Fall taugt
...
directly.  Rather, they combine so-called \emph{node factories}, of which there is one for each basis.
...
The factory of the final bases tree can then be turned into an actual basis by a call to
\begin{lstlisting}
template <class GridView, class FactoryTag>
auto makeBasis(const GridView& gridView, FactoryTag&& factoryTag);
\end{lstlisting}
This method takes a basis factory, and produces a complete basis from it.
...
\todosander{Was passiert wenn ich eine Basis nicht mit dem Default-Konstruktor erzeugen möchte?
  Kann ich dem \cpp{FactoryTag} dann Argumente mitgeben?}

Each basis implementation listed above has a corresponding factory tag, defined in the same header.
Some of them have template parameters to transmit compile-time information.  The tags are
...
auto power(SubFactoryTag&& tag)
\end{lstlisting}
provided in the file \file{dune/functions/functionspacebases/powerbasis.hh}.
It combines \cpp{k} copies of a subtree of type \cpp{SubFactoryTag} in a new tree.  Therefore, the
...
(formally known as node-factory). A pre-basis can be
...
\begin{lstlisting}
  ...
  using PreBasis = PowerNodeFactory<MI, S, ChildPreBasis, 3>;
...
\begin{lstlisting}
  ...
  using PreBasis = CompositeNodeFactory<MI, S, ChildPreBasis1, ChildPreBasis2>;
...
Of course \texttt{PowerNodeFactory} and \texttt{CompositeNodeFactory}
can be nested which allows to construct more complex spaces.
As a basis for the construction of these spaces, the bases
...
Combining different \texttt{PQkNodeFactory}'s one can for
...
\begin{lstlisting}
  ...
  using VelocityPreBasis = PowerNodeFactory<MI, S, SVCPrebasis, dim>;

  // Pre-basis for the pressure
  using PressurePreBasis = PQkNodalBasis<GridView, 1, MI>;

  // Taylor-Hood pre-basis
  using TaylorHoodPreBasis = CompositeNodeFactory<MI, S, VelocityPreBasis, PressurePreBasis>;
...
the \texttt{makeBasis()} are only needed for implementors providing
...
\begin{lstlisting}
  namespace BasisBuilder {
    ...
    template<std::size_t k, class SubPreBasisFactory, class Strategy>
    auto power(SubPreBasisFactory&&, const Strategy&);

    // Create PreBasisFactory for product space with k-times
    // the same child and given default index merging strategy
    template<std::size_t k, class SubPreBasisFactory>
    auto power(SubPreBasisFactory&& subFactory);

    // Create PreBasisFactory for product space with different
    // children and given index merging strategy
    template<class SubPreBasisFactory1, class SubPreBasisFactory2, ..., class Strategy>
    auto composite(const SubPreBasisFactory2&&, SubPreBasisFactory2&&, ..., const Strategy&);

    // Create PreBasisFactory for product space with different
    // children and default index merging strategy
    template<class SubPreBasisFactory1, class SubPreBasisFactory2, ...>
    auto composite(const SubPreBasisFactory2&&, SubPreBasisFactory2&&, ...);

  }
\end{lstlisting}

\todograeser{Describe available index merging strategies}
\todograeser{Rename classes according to naming convention in manual.}
