...
\todograeser{Ist doch nicht so einfach, weil die Knoten-Basisklassen kein CRTP benutzen.}
\begin{lstlisting}[style=Interface]
...
\todosander{Den folgenden Textblock finde ich ziemlich kryptisch.  Ich vermute auch
dass er an einer anderen Stelle besser aufgehoben wäre, aber ich weiß gerade
nicht recht wo.}
\todograeser{Meiner Meinung nach ist das genau der richtige Platz.
Vielleicht hilft ein Beispiel, daß etwas anschaulicher zu machen.
Andererseits bin ich mir nicht sicher, ob man wirklich die momentane
\cpp{DefaultNodeToRangeMap} will oder lieber diese:
\url{https://gitlab.dune-project.org/staging/dune-functions/issues/22}.
Die wäre u.U. intuitiver und auch einfacher zu beschreiben.}
...
for the Stokes problem in the literature.
\todosander{Quellenangabe?}
...
requires handling two indices together.  At the time of writing \dunemodule{dune-functions}
does not provide matrix backends.  Instead, the example code uses a simple helper method
...
\todograeser{Wenn wir \cpp{matrixEntry} hier wirklich beschreiben, würde ich mich als Leser
fragen, was die zusaätzlichen \cpp{[0]} Aufrufe sollen.}
