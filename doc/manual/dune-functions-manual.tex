...
Let $V$ and $W$ be two scalar function space bases.
Then we define the product of the $V$ and $W$ as
\begin{align*}
  V \times W
    \colonequals \big\{ (v,w) \st v \in V, \; w \in W \big\}.
\end{align*}
Functions from this space take values in $\R^{n_1} \times \R^{n_2} = \R^{n_1 + n_2}$.
...
\end{equation}
is the natural basis of the space $\operatorname{span} B \times \operatorname{span} C$.  Its basis functions take values in $\R^{m_1} \times \R^{m_2} = \R^{m_1+m_2}$.
