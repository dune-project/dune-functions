...
These differences separate multi-index
implementations for the different types of trees.  We discuss both types of trees in turn.
...
HybridTreePath<Dune::index_constant<1>, std::size_t> i01(_0,1);
HybridTreePath<Dune::index_constant<2>, std::size_t> i02(_0,2);
\end{lstlisting}
...
\end{lstlisting}
which creates a \cpp{HybridTreePath} with the given entry types.
As this is a free method rather than a constructor, the entries have to be
given only once, and their types are infered.
...
\end{lstlisting}
%
Depending on the template parameter \cpp{i}, the returntype is either \cpp{std::size_t}
or \cpp{Dune::index_constant<i>}.  In the latter case, it can be used
...
\todosander{Überzeugt mich nicht ganz.  Ja, der Rückgabetyp hängt von \cpp{i} ab,
aber alle möglichen Rückgabetypen sind immer nur statische Versionen von \cpp{size_t}.
Wie wäre es mit: As objects of type \cpp{HybridTreePath}
typically hold some dynamic data, they cannot be \cpp{constexpr}.}
\todograeser{Aber das stimmt nicht. Der Grund ist, dass \cpp{constexpr}
einfach etwas grundlegend anderes tut. Deshalb glaube ich auch nicht, dass wir
...
\todosander{Hier währe ein kurzes Beispiel nett, das zeigt, wie die Indizes
in solchen Teilbäumen aussehen.}

These bases can be combined with
