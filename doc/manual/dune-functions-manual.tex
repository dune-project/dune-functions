...
\begin{figure}
 \begin{center}
  ...
  % The occupation pattern
  % Diagonal entries---those have to be there
  \foreach \x in {0,...,9}
  ...
  \foreach \blocknumber in {0,...,9}
  {
    \pgfmathsetmacro{\x}{int(random(0,9))}
    \pgfmathsetmacro{\y}{int(random(\x,9))}
    \fill [lightgray] (0+3*\x,40-3*\y) rectangle (3+3*\x,37-3*\y);  % lower triangular part
    \fill [lightgray] (0+3*\y,40-3*\x) rectangle (3+3*\y,37-3*\x);  % upper triangular part
  }
  ...
  \foreach \blocknumber in {0,...,9}
  {
    ...
    \fill [lightgray] (0+3*\x,\y) rectangle (3+3*\x,1+\y);  % lower left block
    \fill [lightgray] (40-\y,40-3*\x) rectangle (39-\y,37-3*\x);  % upper right block
  }

  % The fine grid
  \foreach \x in {0,...,10}
    \draw [line width=0.05mm] (3*\x,0)--(3*\x,40);

  \foreach \y in {0,...,10}
    \draw [line width=0.05mm] (0,40-3*\y)--(40,40-3*\y);

  \foreach \x in {0,...,9}
    \draw [line width=0.05mm] (40 - 1*\x,0)--(40 - 1*\x,40);
