...
this tree, any interior node allows to access
...
(which is a short cut for \cpp{std::integral_constant<std::size_t, i>}).
...
\begin{lstlisting}[style=Interface]
// The following methods only exists for interior nodes
...
tree.
\todosander{Mir scheint der folgende Satz unpräzise.  Man sollte ihn in der Sprache
von Kapitel~1 umschreiben.}
...
\end{lstlisting}
\todosander{Warum steht hier \cpp{implementation defined} bei \cpp{TreePath}?
Ein \cpp{TreePath} ist doch immer ein \cpp{TypeTree::HybridTreePath}.}
