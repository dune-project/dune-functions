...
of such spaces.
Let $B = \{b_i\}_{i=1}^{n_1}$ and $C = \{c_i\}_{i=1}^{n_2}$ be two scalar function space bases
...
This time, the individual basis functions form the tree leafs, and irreducible bases are
...
\begin{center}
    \begin{tikzpicture}[
        ...
        \node [treenode] {$B$}
            ...
            child{ node [treenode] {$b_2$} }
            child{ node [treenode] {$b_3$} }
            child{ node [] {$\dots$} }
            child{ node [treenode] {$b_{n-1}$} }
            child{ node [treenode] {$b_n$} };
    \end{tikzpicture}
\end{center}
