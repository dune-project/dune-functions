...
have a fixed length.  For example, let's describe a possible indexing of the
basis vectors of the Taylor--Hood space $(P_2 \times P_2 \times P_2) \times P_1$.
We start by assuming that the finite element basis functions $\lambda_i^2$ and $\lambda_k^1$
of $P_2$ and $P_1$ both
have a flat indexing by natural numbers $i=1,\dots,n_2$ and $k=1,\dots,n_1$,
respectively.
Then we enumerate the three coefficients of the velocity
with the numbers $0$, $1$, and $2$.  Further, we denote velocity components with a $0$,
and pressure components with a $1$.
...
\todosander{Einheitlichkeit: Nehmen wir runde oder eckige Klammern für die Multi-Indizes?}

It is evident that the complete set of multi-indices can again be interpreted as a tree,
shown in Figure~\ref{fig:taylor_hood_index_tree}.  In this tree, the multi-index digits
label the \emph{edges} of the tree, and multi-indices are the leaf nodes.  Prefixes can be
...
\todosander{Wofür brauchen wir Uniformität?  Gilt die nicht ohnehin automatisch, da wir ja ein
  endliches $M$ annehmen?}
\end{definition}
...
directly attached to a single root.
...
While the set of all such maps is large, \dunemodule{dune-functions} allows to construct the most
...
Remember first that we assume that basis trees are ordered, i.e., the children of each tree node
have a fixed given ordering. We then decorate each inner node $A$ of the basis tree with a rule
that specifies how the index trees of the basis subtrees are combined to form an index tree
...
\begin{itemize}
 ...
 \item \textbf{FlatLexicographic}: Instead of introducing a new root node, merge the existing
   ...
   \todosander{Hier müssen jetzt irgendwie Offsets ins Spiel kommen.}
\end{itemize}
...
\begin{itemize}
 \item FlatInterleaved
 \item LeafBlockedInterleaved
\end{itemize}
