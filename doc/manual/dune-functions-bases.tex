...
The \dunemodule{dune\-functions} module uses \cpp{Dune::TypeTree::TreePath}
for this, which we describe in detail in Section~\ref{sec:multi_indices}.
Using a \cpp{TreePath} object, the example looks as follows:
%
\begin{lstlisting}[style=Example]
...
Conceptually, a \cpp{TreePath} is a fixed-size container, where each entry
...
An object of type \cpp{TreePath} can be used to access the nodes of a
...
of the type
\begin{lstlisting}[style=Example]
TreePath<Dune::index_constant<0>, std::size_t>;
\end{lstlisting}
whereas the multi-index for the pressure leaf node would use the type
\begin{lstlisting}[style=Example]
TreePath<Dune::index_constant<1>>;
\end{lstlisting}
To construct objects of these types, call
\begin{lstlisting}[style=Example]
using namespace Dune::Indices;
TreePath<Dune::index_constant<0>, std::size_t> i00(_0,0);
TreePath<Dune::index_constant<0>, std::size_t> i01(_0,1);
TreePath<Dune::index_constant<0>, std::size_t> i02(_0,2);
\end{lstlisting}
for the velocity leaf nodes, and
\begin{lstlisting}[style=Example]
TreePath<Dune::index_constant<1>> i1(_1);
\end{lstlisting}
...
\end{lstlisting}
which creates a \cpp{TreePath} with the given entries.
...
To access entries of a \cpp{TreePath} object, the object has
the method
%
\begin{lstlisting}[style=Interface]
...
\cpp{constexpr}:
Since \cpp{TreePath} objects are intended to select
...
These are just the more important methods of the \cpp{TreePath} class.
...
\begin{lstlisting}[style=Interface]
using PrefixPath = TypeTree::TreePath@@<implementation defined>@@;
...
as a single \cpp{TreePath} object (see Section~\ref{sec:multi_indices}), or
...
auto subspaceBasis(const RootBasis& rootBasis,
                   const TypeTree::TreePath<PathIndices...>& prefixPath);
