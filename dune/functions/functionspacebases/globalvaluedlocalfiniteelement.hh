...
namespace Dune::Functions::Impl
{
  ...
  struct ContravariantPiolaTransformator
  {
    ...
    static auto apply(Values&& valuesLocal,
    ...
    static auto applyJacobian(Gradients&& gradientsLocal,
                              const LocalCoordinate& xi,
                              const Geometry& geometry)
    {
      auto jacobianTransposed = geometry.jacobianTransposed(xi);
      ...
      std::decay_t<Gradients> gradients(gradientsLocal.size());

      for (std::size_t i=0; i<gradientsLocal.size(); i++)
      {

        for (size_t j=0; j<gradients[i].N(); j++)
          for (size_t k=0; k<gradients[i].M(); k++)
          {
            gradients[i][j][k] = 0;
            for (size_t l=0; l<gradientsLocal[i].N(); l++)
                gradients[i][j][k] += jacobianTransposed[l][j] * gradientsLocal[i][l][k];
            gradients[i][j][k] /= integrationElement;
          }
  ...
  public:
                          ...
                          std::vector<typename Traits::RangeType>& out) const
    {
      ...
      out = Transformator::apply(out, x, element_->geometry());
    }
                          ...
                          std::vector<typename Traits::JacobianType>& out) const
    {
      ...
      out = Transformator::applyJacobian(out, x, element_->geometry());
    }
                 ...
                 std::vector<typename Traits::RangeType>& out) const
    {
      ...
      if (totalOrder == 0) {
        ...
        fullJacobian = Transformator::applyJacobian(fullJacobian, x, element_->geometry());
